...
function createImageProxyMiddleware(config, apiClient = null) {
  ...
  return async (req, res) => {
    ...
    try {
      ...
      const imageData = await _fetchImageWithCache(videoId, type, cdnImageUrl, imageFetchService);

      // Send response
      _sendImageResponse(res, imageData, config.cache.browserCache);
    } catch (err) {
 ...
 */
function _sendImageResponse(res, imageData, enableBrowserCache) {
 ...
 * @private
 */
async function _fetchImageWithCache(videoId, imageType, cdnUrl, imageFetchService) {
