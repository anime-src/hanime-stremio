 ...
 * Uses direct store access with write-back pattern (memory + Redis) for persistence
 */
...
const USER_SESSION_KEY_PREFIX = 'user:';
...
class UserApiManager {
  ...
  /**
   * Create cache stores for user sessions
   * Uses same pattern as lib/cache.js: memory store (L1) + Redis (L2)
  ...
  _createSessionCache() {
    ...
    const memoryStore = createKeyv({ 
      ...
      lruSize: config.cache.maxSize || 5000 
    });
  ...
  clearCache(email, password) {
    if (!email || !password || !this.sessionCache) {
      return;
    }
    ...
    const stores = this.sessionCache.stores || [];
    const l1Store = stores[0];
    const l2Store = stores[1];

    // Delete from both stores
    if (l1Store) {
      l1Store.delete(cacheKey).catch((err) => {
        logger.warn('Error clearing user session cache from L1', {
          error: err.message
        });
      });
    }
    ...
    if (l2Store) {
      l2Store.delete(cacheKey).catch((err) => {
        logger.warn('Error clearing user session cache from L2', {
          error: err.message
        });
      });
    }
  ...
  clearAllCache() {
    ...
    // Note: We can't easily clear all entries from stores without iterating
    // Entries will expire automatically via TTL
  ...
  destroy() {
    this.initPromises.clear();
    // Note: Keyv stores handle their own cleanup
  ...
  async _getCachedUserApi(credentialsHash, email, password) {
    ...
    try {
      ...
      if (l1Store) {
        try {
          cached = await l1Store.get(cacheKey);
          if (cached) {
            logger.debug('User session cache hit L1', {
              emailPrefix: email.substring(0, 3) + '***'
            });
          }
        } catch (e) {
          // L1 error, continue to L2
          if (logger && logger.debug) logger.debug(`L1 get failed key=${cacheKey} err=${e.message}`);
        }
      }

      // Try L2 (Redis) if not found in L1
      if (!cached && l2Store) {
        try {
          cached = await l2Store.get(cacheKey);
          if (cached) {
            logger.debug('User session cache hit L2', {
              emailPrefix: email.substring(0, 3) + '***'
            });
            // Promote to L1 asynchronously (don't block)
            if (l1Store) {
              // Calculate remaining TTL
              const remainingTtl = cached.expiresAt ? Math.max(0, cached.expiresAt - Date.now()) : 12 * 60 * 60 * 1000;
              l1Store.set(cacheKey, cached, remainingTtl).catch((err) => {
                if (logger && logger.debug) logger.debug(`L1 promotion failed key=${cacheKey} err=${err.message}`);
              });
            }
          }
        } catch (e) {
          // L2 error, continue
          if (logger && logger.debug) logger.debug(`L2 get failed key=${cacheKey} err=${e.message}`);
        }
      }
      ...
      if (cached.expiresAt && Date.now() > cached.expiresAt) {
        ...
        // Delete from both stores
        if (l1Store) {
          l1Store.delete(cacheKey).catch(() => {});
        }
        if (l2Store) {
          l2Store.delete(cacheKey).catch(() => {});
        }
        return null;
      }
  ...
  async _initializeUserApi(email, password) {
    try {
      ...
      // Store serializable session data in cache (if enabled) - using write-back pattern
      if (this.sessionCache) {
        try {
          const credentialsHash = this._getCredentialsHash(email, password);
          const cacheKey = this._getCacheKey(credentialsHash);
          ...
          if (ttl > 0) {
            ...
            if (l1Store) {
              await l1Store.set(cacheKey, sessionData, ttl);
              logger.debug('Stored user session in L1 cache', {
                emailPrefix: email.substring(0, 3) + '***',
            ...
            if (l2Store) {
              l2Store.set(cacheKey, sessionData, ttl).catch((err) => {
                // Silently handle L2 write failures (non-critical)
                if (logger && logger.debug) logger.debug(`L2 write failed key=${cacheKey} err=${err.message}`);
              });
            }
          } else {
