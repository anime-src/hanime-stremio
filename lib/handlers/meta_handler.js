...
class MetaHandler {
  ...
  async handle(args) {
    try {
      ...
      // Reject non-Hanime IDs (e.g., IMDB IDs like tt4508902)
      if (!isHanimeId(id)) {
        this.logger.debug('Meta handler: ignoring non-Hanime ID', { id, type: args.type });
        return { meta: null };
      }
      ...
      if (this.cache) {
        const metaTtl = this.config.cache.metaTtl || this.config.cache.ttl;
        this.cache.set(cacheKey, response, metaTtl);
      }
