/**
 * Series Service
 * Detects and groups multi-episode content into series
 */
...
class SeriesService {
   ...
   */
  static generateSeriesSlug(baseName) {
    return baseName
      .toLowerCase()
      .replace(/[^a-z0-9\s-]/g, '')
      .replace(/\s+/g, '-')
  ...
  static detectSeries(videos) {
    ...
    videos.forEach(video => {
      const episodeInfo = this.parseEpisodeInfo(video.name);
    ...
    seriesMap.forEach((seriesData, baseName) => {
      ...
      if (seriesData.episodes.length >= 2) {
        ...
        series.push({
          id: `series:${this.generateSeriesSlug(baseName)}`,
          baseName: baseName,
   ...
   */
  static getSeriesCatalogItems(videos) {
    const series = this.detectSeries(videos);
   ...
   */
  static getSeriesEpisodes(videos, seriesSlug) {
