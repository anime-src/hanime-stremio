...
const SeriesService = require('./series_service');
...
class MetaService {
  ...
  async getMetaData(id) {
    ...
    return cacheWrapMeta(id, async () => {
      ...
      if (isSeries) {
        return await this.getSeriesMeta(strippedId, publicUrl);
      } else {
        return await this.getVideoMeta(strippedId, publicUrl);
      }
    });
  }
   ...
   */
  async getVideoMeta(strippedId, publicUrl) {
    ...
    const meta = TransformService.toStremioMeta(video, imageUrls, publicUrl);
   ...
   */
  async getSeriesMeta(seriesId, publicUrl) {
    try {
      ...
      const prefixedSeriesId = `${constants.addonPrefix}:${seriesId}`;
