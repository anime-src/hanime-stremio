...
const config = {
  ...
  cache: {
    ...
    upstashUrl: process.env.CACHE_UPSTASH_REDIS_URL || process.env.UPSTASH_REDIS_URL || null, // Upstash Redis URL for persistent cache (optional, highest priority)
    upstashToken: process.env.CACHE_UPSTASH_REDIS_TOKEN || process.env.UPSTASH_REDIS_TOKEN || null, // Upstash Redis token for persistent cache (optional, highest priority)
    redisUrl: process.env.CACHE_REDIS_URL || process.env.REDIS_URL || null, // Redis connection URL for persistent cache (optional, takes priority over PostgreSQL)
    postgresUrl: process.env.CACHE_POSTGRESQL_URL || process.env.POSTGRESQL_URL || process.env.NETLIFY_DATABASE_URL || null, // PostgreSQL connection URL for persistent cache (optional)
    ttl: {
  ...
  slowDown: {
    ...
    general: {
      ...
      delayAfter: parseInt(process.env.SLOW_DOWN_DELAY_AFTER, 10) || 150, // Start slowing down after 150 requests (closer to rate limit)
    ...
    // Strategy: Start slowing at 45, rate limit at 60 (smaller gap)
    imageProxy: {
      windowMs: parseInt(process.env.SLOW_DOWN_IMAGE_WINDOW_MS, 10) || 15 * 60 * 1000, // 15 minutes
      delayAfter: parseInt(process.env.SLOW_DOWN_IMAGE_DELAY_AFTER, 10) || 45, // Start slowing down after 45 requests (closer to rate limit)
