...
const config = {
  ...
  api: {
    ...
    user: {
      email: process.env.HANIME_EMAIL || process.env.HANIME_USER || 'peric.danijel@gmail.com',
      password: process.env.HANIME_PASSWORD || process.env.HANIME_PASS || 'Djelatn1k'
    }
  },
...
/**
 * Merge user configuration (from Stremio config page) with base config
 * User config overrides environment variables
 * @param {Object} userConfig - User configuration from args.config (email, password)
 * @returns {Object} Merged configuration object
 */
function mergeConfig(userConfig = {}) {
  const merged = JSON.parse(JSON.stringify(config)); // Deep clone

  // Override user credentials if provided via config page
  if (userConfig.email || userConfig.password) {
    merged.api.user = {
      email: userConfig.email || config.api.user.email,
      password: userConfig.password || config.api.user.password
    };
  }
